...
public class StrawberryDimensionTeleporter
{
    ...
    public StrawberryDimensionTeleporter(WorldServer par1WorldServer)
    {
    ...
    {
        if (this.worldServerInstance.provider.dimensionId != testMod.strawberryDimensionId)
        {
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlock(k1, l1, i2, flag ? testMod.fireLog.blockID : 0);
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
    {
        ...
        if (d3 >= 0.0D)
        {
            if (flag)
            {
                this.destinationCoordinateCache.add(j1, new StrawberryPortalPosition(this, i, j, k, this.worldServerInstance.getTotalWorldTime()));
            ...
            for (k2 = -1; k2 <= 1; ++k2)
            {
                for (i3 = 1; i3 < 3; ++i3)
                {
                    for (l2 = -1; l2 < 3; ++l2)
                    {
                        ...
                        i4 = j2 + (i3 - 1) * l5 - k2 * k5;
                        flag = l2 < 0;
                        this.worldServerInstance.setBlock(k3, j3, i4, flag ? testMod.fireLog.blockID : 0);
                    }
                }
            }
        }
        ...
        for (k2 = 0; k2 < 4; ++k2)
        {
            for (i3 = 0; i3 < 4; ++i3)
            {
                for (l2 = -1; l2 < 4; ++l2)
                {
                    ...
                    this.worldServerInstance.setBlock(k3, j3, i4, flag ? testMod.fireLog.blockID : Block.portal.blockID, 0, 2);
                }
            }
